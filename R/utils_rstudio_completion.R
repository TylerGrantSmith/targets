# taken from https://github.com/EmilHvitfeldt/paletteer/blob/master/R/completeme.R
...
current_function <- function(env) {
  utils:::inFunction(env[["linebuffer"]], env[["start"]])
}
...
targets_completer <- function(env, store = targets::tar_config_get("store")) {
  
  funs <- c("tar_load", "tar_load_raw", "tar_read", "tar_read_raw")
  
  if (!(current_function(env) %in% funs && is_first_argument(env))) {
    return(NULL)
  }
  ...
  out <- meta$database$read_condensed_data()
  names <- out[["name"]]
  text <- env[["token"]]
  names[grepl(text, names)]
}

.onLoad <- function(lib,pkg) {
  rc.options(custom.completer = completeme)
  register_completion(thispkg = targets_completer)
}
